...
void printStatistics(const Estimator &estimator, double t)
{
    if (estimator.solver_flag != Estimator::SolverFlag::NON_LINEAR)
        return;
    printf("position: %f, %f, %f\r", estimator.Ps[WINDOW_SIZE].x() * 1000, estimator.Ps[WINDOW_SIZE].y() * 1000, estimator.Ps[WINDOW_SIZE].z() * 1000);
    
        //ROS_DEBUG("calibration result for camera %d", i);
        ROS_DEBUG_STREAM("extirnsic tic: " << estimator.tic.transpose());
        ROS_DEBUG_STREAM("extrinsic ric: " << Utility::R2ypr(estimator.ric).transpose());
        if (ESTIMATE_EXTRINSIC)
        {
            Eigen::Matrix3d eigen_R;
            Eigen::Vector3d eigen_T;
            eigen_R = estimator.ric;
            eigen_T = estimator.tic;
            cv::Mat cv_R, cv_T;
...
void pubPointCloud(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    for (auto &it_per_id : estimator.f_manager.feature)
    {
        ...
        Vector3d w_pts_i = estimator.Rs[imu_i] * (estimator.ric * pts_i + estimator.tic) + estimator.Ps[imu_i];
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        for (auto &it_per_id : estimator.f_manager.feature)
        {
            ...
            if(it_per_id.start_frame < WINDOW_SIZE - 2 && it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2 && it_per_id.solve_flag == 1)
            {
                ...
                int imu_i = it_per_id.start_frame;
                Vector3d pts_i = it_per_id.feature_per_frame[0].point * it_per_id.estimated_depth;
                Vector3d w_pts_i = estimator.Rs[imu_i] * (estimator.ric * pts_i + estimator.tic)
                                      + estimator.Ps[imu_i];
                geometry_msgs::Point32 p;
                p.x = w_pts_i(0);
                p.y = w_pts_i(1);
